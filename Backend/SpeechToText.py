...
# load environmment variablles from the .env file.
...
def get_driver():
    """Get or create a valid WebDriver instance."""
    global driver
    ...
    def is_driver_valid(d):
        """Check if driver session is still valid."""
        if d is None:
            return False
        try:
            # Try to access a property - will fail if session is invalid
            _ = d.session_id
            _ = d.current_url
            return True
        except Exception:
...
def QueryModifier(Query):
    ...
    # Chech if the query is a question and add a question mark it necessary.
...
# Function to perfrom speech recognition using the WebDriver.
def SpeechRecognition():
    ...
    for attempt in range(max_retries):
        try:
            # Get a valid driver instance
            active_driver = get_driver()
            # Open the HTMl file in the browser.
            active_driver.get("file:///" + Link)
            # Start speech recogniton by clicking the start button.
            active_driver.find_element(by=By.ID, value="start").click()

            while True:
                try:
                    # Get the recognized text from the HTMl output element.
                    Text = active_driver.find_element(by=By.ID, value="output").text

                    if Text:
                        # Stop recognition by clicking the stop button.
                        active_driver.find_element(by=By.ID, value="end").click()
                    ...
                    # Check if it's a session error - if so, break to retry
            ...
            print(f"Speech recognition attempt {attempt + 1} failed: {e}")
            driver = None  # Reset driver to force recreation
